...
class Resolutions<T extends Path> extends $Resolutions<T> {
  Resolutions({BuiltMap<Reference, T> data})
      : __data = data,
        assert(data != null);

  Iterable<T> _datas;

  final BuiltMap<Reference, T> __data;

  Resolutions<T> clone({BuiltMap<Reference, T> data}) {
    return Resolutions(
      data: data ?? __data,
    );
  }
  ...
  bool operator ==(dynamic other) {
    if (identical(other, this)) return true;
    if (other is! Resolutions) return false;
    return _data == other._data;
  }

  String toString() {
    return "Resolutions (_data: $_data)";
  }
}
