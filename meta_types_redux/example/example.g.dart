...
// **************************************************************************
// _MetaTypesReduxGenerator
// **************************************************************************

abstract class CounterActionsActions implements _CounterActionsBase {
  Counter reduce(
    Counter state, {
    @required Counter increment(Counter state, int increment),
    @required Counter decrement(Counter state, int decrement),
  }) {
    if (incrementSet) return increment(state, this.increment);
    return decrement(state, this.decrement);
  }
  ...
  FutureOr<void> handleMiddleware(
    ...
    @required
        FutureOr<void> increment(MiddlewareApi<Counter, CounterActions> api,
            MetaNextDispatcher next, int increment),
    @required
        FutureOr<void> decrement(MiddlewareApi<Counter, CounterActions> api,
            MetaNextDispatcher next, int decrement),
  }) {
    if (incrementSet) return increment(api, next, this.increment);
    return decrement(api, next, this.decrement);
  }
}
